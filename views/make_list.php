    ...
    function createRows(data, obj) {
        ...
        if (jQuery(data).length > 0) {
            jQuery(data).each(function (key, val) {
                ...
                html += '<tr><td class="chkInn"><label><input type="checkbox" class="chkSelect checkBox" id="chkSelect" value="' + val.id + '"  name="chkSelect[' + val.id + ']" /><span><i class="fa fa-check" aria-hidden="true"></i></label></td><td class="text_left checkId" data-title="' + val.id + '">' + val.make + '</td><td class="statusCell" data-title="Status:"><span class="statusSpan ' + (val.status == '0' ? "inactive" : "") + '" title="' + (val.status == '1' ? "Active" : "Inactive") + '"></span> ' + (val.status == '1' ? "Active" : "Inactive") + '</td><td class="S_action" data-title="Actions:"><a href="' + window.location.href.split('?')[0].replace(/list/g, "add") + '?id=' + val.id + '&page=' + getCurrentPage() + '" class="tEdit trans"><i class="fa fa-pencil-square-o trans"  title="Edit"  aria-hidden="true"></i></a><a class="delRecord" data-title="' + val.id + '" href="javascript:;" title="Delete" class="tDelete trans"><i class="fa fa-trash-o fa-1x trans" aria-hidden="true"></i></a><a class="chStatus" data-title="' + val.id + '" title="Change status" data-status="' + (val.status == '1' ? "0" : "1") + '" href="javascript:;" class="tStatus trans"><i class="fa fa-exchange fa-1x trans" aria-hidden="true"></i></a>' + restoreHTML(val.id) + '</td></tr>';
            });
        } else {
                                ...
                                <div class="teamHeader">
                                    ...
                                    <div class="searchSec">
                                        ...
                                        <form id="searchFrm" name="searchFrm" onsubmit="return false;">
                                            <input type="text" title="Please enter name" id="txtSearch" name="search" class="inputFils" />
                                    ...
                                    <div class="S_addNew"> 
                                        <a href="<?php echo setLink('user/make_add'); ?>" title="Add New" class="btn skyBlueBtn">Add New</a>
                                    </div>
                                </div>
