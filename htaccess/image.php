...
///////////////////// 
$absPath =__DIR__;
define('BASEPATH', $absPath);
include  $absPath."/application/config/config.php";
// Set flag for restricting direct access to other files. 
$mediaPath=$absPath."/media/worthyparts/".$_GET['userid']."/".$_GET['mp_id']."/".$_GET['module']."/";  
        
if($_GET['userid']=="blogext"){ //Logic for display blog image on home page
    $mediaPath=$absPath."/blog/wp-content/uploads/".$_GET['mp_id']."/".$_GET['module']."/";  
    $_GET['module']=$_GET['userid'];
}
//$noImage="./themes/hello_theme/images/no-image.jpg";
$noImage=$absPath."/media/worthyparts/no-image.jpg"; 
$qs = $_GET['image']; 
// get extension
$arrExt = explode(".",$qs);
...
*/
$fileName=$mediaPath.$imgName.".".$ext;          
if(isset($_POST['del']) && file_exists($fileName)){
    unlink($fileName);
    echo "success";
    exit;
}
...
list($width,$height)=$config[$_GET['module']][$_GET['size']];
$_GET['image']=$fileName;
$_GET['width'] =$width;
$_GET['height'] =$height;
